    ...
    <Dialog>
      <DialogTrigger>
        <div className="flex">
          <div
            className={cn(
              "border-x-2 px-2 py-4 flex-1 cursor-pointer hover:bg-gray-200 duration-100",
              {
          ...
          >
            <div className="flex items-center justify-between">
              <p className="font-bold">{chunk.name}</p>
              <p
                className={cn(
                  "font-semibold lowercase flex items-center gap-2",
                  {
              ...
              >
                {chunk.type}
                {chunk.type === "OTHER" ? (
                  <FileQuestion className="w-4 h-4 text-rosyBrown" />
                ) : chunk.type === "SUCCESS" ? (
                  <Check className="w-4 h-4 text-darkCyan" />
                ) : (
                  <X className="w-4 h-4 text-darkPurple" />
                )}
              </p>
              <p className="font-semibold text-sm">
                {formatPrismaDateToRelativeTime(chunk.createdAt)}
              </p>
            </div>
          </div>
          <div
            onClick={() => handleDelete(chunk.id)}
            className="bg-gray-300 p-2 rounded-r-md cursor-pointer group hover:rounded-none duration-100 flex items-center"
          >
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{chunk.name}</DialogTitle>
        </DialogHeader>
        <DisplayObject obj={JSON.parse(chunk.data)} />
      </DialogContent>
    </Dialog>
  );
};
